$(document).ready(function () {
  ...
  $('#search').on('keyup',function(){
    ...
    $('#reminder_table tr').each(function(){
        ...
        if(lineStr.indexOf(searchTerm) === -1){
          if ($(this).attr('class') != 'table_header'){$(this).hide();}
        }else{
           $(this).show();
        }
    });
  ...
  //     if (jqXHR.status === 204) {
  //       console.log(form)
  //       action = form.attr('action').replace( 'complete', 'incomplete' );
  //       form.removeClass('complete');
  //       form.addClass('incomplete');
  //       form.attr('action', action);
  //       form.html("<button type='submit'>Incomplete</button>");
  //       form.parents("tr").addClass('complete');
  //     } else if (jqXHR.status === 200) {
  //       document.location = data;
  //     }
  //   });
